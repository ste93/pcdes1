...
public class PlanetManager {
	...
	public void updatePlanetsAcceleration(int planet1Number, int planet2Number) {
		...
		if(deltaY != 0) {
			...
			planetA.updateAccelerationX(BigDecimal.valueOf(accelerationAX * (deltaX > 0 ? -1 : 1)));
			planetB.updateAccelerationX(BigDecimal.valueOf(accelerationBX * (deltaX < 0 ? -1 : 1)));
			planetA.updateAccelerationY(BigDecimal.valueOf(accelerationAY * (deltaY > 0 ? -1 : 1)));
			planetB.updateAccelerationY(BigDecimal.valueOf(accelerationBY  * (deltaY < 0 ? -1 : 1)));
		} else {
			planetA.updateAccelerationX(BigDecimal.valueOf(accelerationA * (deltaX > 0 ? -1 : 1)));
			planetB.updateAccelerationX(BigDecimal.valueOf(accelerationB * (deltaX < 0 ? -1 : 1)));
			planetA.updateAccelerationY(BigDecimal.ZERO);
			planetB.updateAccelerationY(BigDecimal.ZERO);
...
/*
	...
	BigDecimal accelerationBY = BigDecimal.valueOf(-1 * planetA.getMassPerGravity())
		...
		planetA.updateAccelerationX(BigDecimal.valueOf(accelerationAX));
		planetB.updateAccelerationX(BigDecimal.valueOf(accelerationBX));
		planetA.updateAccelerationY(BigDecimal.valueOf(accelerationAY));
		planetB.updateAccelerationY(BigDecimal.valueOf(accelerationBY));
		*/
    ...
    public void updatePlanetPosition(int planetNumber) {
		...
		if(newPositionX > Constants.MAX_POSITION_X) {
			...
			//TODO remove
			System.out.println("new position X lower than 1");
			if (planet.getAccelerationX().compareTo(BigDecimal.ZERO) < 0)
				planet.invertAccelerationX();
			if (planet.getSpeedX().compareTo(BigDecimal.ZERO) < 0)
				planet.setSpeedX(planet.getSpeedX().negate());
		...
		if(newPositionY > Constants.MAX_POSITION_Y) {
			...
			//TODO remove
			System.out.println("new position Y lower than 1");
			if (planet.getAccelerationY().compareTo(BigDecimal.ZERO) < 0)
				planet.invertAccelerationY();
			if (planet.getSpeedY().compareTo(BigDecimal.ZERO) < 0)
				planet.setSpeedY(planet.getSpeedY().negate());
